package com.risevision.ui.client.common;

public class ContentId {
	//HOME
	public static String HOME = "HOME";
	//SETTINGS
	public static String COMPANIES = "COMPANIES";
	public static String COMPANY_MANAGE = "COMPANY_MANAGE";
	public static String USERS = "USERS";
	public static String USER_MANAGE = "USER_MANAGE";
	//CONTENT
	public static String SCHEDULES = "SCHEDULES";
	public static String SCHEDULE_MANAGE = "SCHEDULE_MANAGE";
	public static String PLAYLISTS = "PLAYLISTS";
	public static String PLAYLIST_MANAGE = "PLAYLIST_MANAGE";
	public static String PRESENTATIONS = "PRESENTATIONS";
	public static String PRESENTATION_MANAGE = "PRESENTATION_MANAGE";
	public static String GADGETS = "GADGETS";
	public static String GADGET_MANAGE = "GADGET_MANAGE";
	//MANAGE
	public static String MANAGE_PORTAL = "MANAGE_PORTAL";
	public static String MANAGE_TERMS = "MANAGE_TERMS";
	public static String MANAGE_SECURITY = "MANAGE_SECURITY";
	//DISPLAYS
	public static String DISPLAYS = "DISPLAYS";
	public static String DISPLAY_MANAGE = "DISPLAY_MANAGE";

}
