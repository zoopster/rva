...
import com.risevision.ui.client.common.widgets.mediaLibrary.CannedAccessControlList;
...
public class AWSAuthConnection {
    ...
    private static final String S3_CANNED_ACL = "x-amz-acl";
	...
	public Response createBucket(String bucket, Map headers)
        throws MalformedURLException, IOException, ServiceFailedException
    {
     ...
     * @throws ServiceFailedException 
     */
    public Response deleteItems(String bucket, ArrayList<String> itemNames, Map headers)
        throws MalformedURLException, IOException, ServiceFailedException
    {
    	byte[] deleteItemsXMLDoc = convertToXmlByteArray(itemNames);
    	
        S3Object object = new S3Object(deleteItemsXMLDoc, null);
        
        Map pathArgs = new HashMap();
        pathArgs.put("delete", null);
        
        String md5Base64 = "";
		try {
			MessageDigest digest = MessageDigest.getInstance("MD5");
			digest.update(deleteItemsXMLDoc);

			byte[] md5 = digest.digest();

			md5Base64 = new String(Base64.encodeBase64(md5));
        ...
        ArrayList header1 = new ArrayList<String>();
        header1.add(md5Base64);
        headers.put("Content-MD5", header1);
        
        ArrayList header2 = new ArrayList<String>();
        header2.add(String.valueOf(deleteItemsXMLDoc.length));
        headers.put("Content-Length", header2);
        
//        ArrayList header3 = new ArrayList<String>();
//        header3.add("application/xml");
//        headers.put("Content-Type", header3);
        
        HttpURLConnection request = makeRequest("POST", bucket, "", pathArgs, headers, object);
	...
	public byte[] convertToXmlByteArray(ArrayList<String> itemNames) {
	...
	private void writeKey(XmlWriter xml, String key) {
		xml.start("Object");
		xml.start("Key").value(key).end();
		// if (version != null) {
		// xml.start("VersionId").value(version).end();
		// }
		xml.end();
	}
    ...
    public Response putACL(String bucket, String key, Map headers)
        throws MalformedURLException, IOException, ServiceFailedException
    {
        Map pathArgs = new HashMap();
        pathArgs.put("acl", null);
        ...
        headers.put(S3_CANNED_ACL, CannedAccessControlList.PublicRead);
