 ...
 * the License.
 */
package com.risevision.gaerequest.server;
...
public class GaeAuthFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest,	ServletResponse servletResponse, FilterChain filterChain)
		...
		if (!request.getRequestURI().contains("loginHelper") && !userService.isUserLoggedIn()) {
			String redirectUrl = request.getHeader(GaeHelper.REDIRECT_URL_HTTP_HEADER_NAME);
			if (redirectUrl == null || redirectUrl.length() == 0) {
				// Default to the root page if the redirecturl isn't specified
				// in the request.
				redirectUrl = "/";
			}
